...
class RenameInputState {
	...
	get blurBehavior() {
		...
		return this.providerState ? 'none' : 'save';
	}
	...
	onInputBlur() {
		// Mode may already be 'view' here — e.g. Escape already called cancel()
		// synchronously, and the resulting unmount of the input triggers a blur too.
		// Re-running save/cancel against a already-settled value would be a no-op at
		// best and an unwanted extra onSave call at worst.
		if (this.mode !== 'edit') return;
		if (this.blurBehavior === 'none') return;
		if (this.blurBehavior === 'save') {
			this.save();
			return;
		}
		this.cancel();

<script lang="ts">
	...
	function colorToIndices(color: string) {
		for (let ci = 0; ci < COLORS.length; ci++) {
			const si = COLORS[ci].shades.indexOf(color);
			if (si !== -1) return { colorIdx: ci, shadeIdx: si };
		}
		return { colorIdx: DEFAULT_COLOR_IDX, shadeIdx: DEFAULT_SHADE };
	}

	// ── Local state ────────────────────────────────────────────────────────────

	let open = $state(false);
	let localColorIdx = $state(DEFAULT_COLOR_IDX);
	let localShadeIdx = $state(DEFAULT_SHADE);

	// Sync local indices from value (e.g. when parent sets a new color)
	$effect(() => {
		const idx = colorToIndices(value);
		localColorIdx = idx.colorIdx;
		localShadeIdx = idx.shadeIdx;
	});

	// Push local state back to value whenever it changes
	$effect(() => {
		value = COLORS[localColorIdx].shades[localShadeIdx];
	});

	const textColor = $derived(localShadeIdx <= 3 ? 'black' : 'white');
</script>

<Popover.Root bind:open>
	<Popover.Trigger
		class="h-8 w-8 shrink-0 rounded-full border-2 border-white shadow-sm ring-1 ring-border transition-transform hover:scale-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
	...
	<Popover.Content class="w-72 p-3" align="start">
		...
		<div class="grid grid-cols-3 gap-1.5">
			{#each COLORS as color, i}
				<button
					type="button"
					onclick={() => (localColorIdx = i)}
					style="background-color: {color.shades[localShadeIdx]}; color: {localShadeIdx <= 3 ? 'black' : 'white'}"
					class="inline-flex items-center justify-center rounded-full px-3 py-1 text-xs font-medium transition-opacity hover:opacity-80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 {localColorIdx === i ? 'ring-2 ring-ring ring-offset-1' : ''}"
				>
					{color.name}
				</button>
			{/each}
		...
		<div class="mt-3 flex flex-col gap-1.5">
			<div class="flex justify-between text-xs text-muted-foreground">
				<span>Light</span>
				<span>{SHADE_STEPS[localShadeIdx]}</span>
				<span>Dark</span>
			</div>
			<Slider type="single" min={0} max={10} step={1} bind:value={localShadeIdx} />
		</div>
		...
		<div class="mt-3 flex justify-center">
			...
			>
				{COLORS[localColorIdx].name} {SHADE_STEPS[localShadeIdx]}
			</span>
		</div>
	</Popover.Content>
</Popover.Root>
